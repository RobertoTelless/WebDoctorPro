...
namespace GEDSys_Presentation.Controllers
{
    public class AreaPacienteController : Controller
    {
        ...
        private List<AREA_PACIENTE> listaMasterArea = new List<AREA_PACIENTE>();
        private AREA_PACIENTE objetoArea = new AREA_PACIENTE();

        public AreaPacienteController(IPacienteAppService baseApps, IConfiguracaoAppService confApps, IUsuarioAppService usuApps, IAcessoMetodoAppService aceApps, IAssinanteAppService assApps, IConfiguracaoCalendarioAppService calApps, IConfiguracaoAnamneseAppService anaApps, ILogAppService logApps, ILocacaoAppService locaApps, IAreaPacienteAppService areaApps, ITemplateEMailAppService temApps, IMensagemEnviadaSistemaAppService meApps, ITemplateSMSAppService smsApps, INoticiaAppService notApps, IEmpresaAppService empApps)
        {
            baseApp = baseApps;
            confApp = confApps;
            usuApp = usuApps;
        ...
        public ActionResult MontarTelaAreaPaciente()
        {
            try
            {
                ...
                if (Session["ListaConsultasGeral"] == null)
                {
                    ...
                    listaMasterConsulta = listaConsulta.Where(p => p.PACO_IN_ATIVO == 1).ToList();
                    listaMasterConsulta = listaMasterConsulta.Where(p => p.PACO_DT_CONSULTA.Date >= DateTime.Today.Date).ToList();
                    ...
                    Session["ListaConsultas"] = listaMasterConsultaFutura;
                    Session["TotConsultasGeral"] = listaMasterConsulta.Count();
                    Session["TotConsultas"] = listaMasterConsultaFutura.Count();
                }
                listaMasterConsulta = (List<PACIENTE_CONSULTA>)Session["ListaConsultasGeral"];
                ViewBag.Consultas = listaMasterConsulta;
                ViewBag.NumConsultas = (Int32)Session["TotConsultasGeral"];
                ViewBag.NumConsultasFuturas = (Int32)Session["TotConsultas"];
                ...
                {
                    listaMasterNoticia = CarregaNoticiaGeral().Where(p => p.NOTC_DT_EMISSAO == DateTime.Today.Date).ToList();
        ...
        public ActionResult ExibirHoje()
        {
            try
            {
                ...
                String cpf = (String)Session["PacienteCPF"];
                List<PACIENTE_CONSULTA> listaConsulta = baseApp.GetConsultasByCPF(cpf);
        ...
        public ActionResult ExibirTodas()
        {
            try
            {
                ...
                String cpf = (String)Session["PacienteCPF"];
                List<PACIENTE_CONSULTA> listaConsulta = baseApp.GetConsultasByCPF(cpf);
                listaMasterConsulta = listaConsulta;
                Session["ListaConsultasGeral"] = listaMasterConsulta;
                return RedirectToAction("MontarTelaAreaPaciente");
            }
            catch (Exception ex)
                ...
                try
                {
                    ...
                    if (Session["FileQueueAreaPaciente"] != null)
                    {
                        ...
                        foreach (var file in fq)
                        {
                            ...
                            {
                                Int32 volta1 = await UploadFileQueueAreaBlob(file);
                            }
                        }
                    ...
                    hist.PAHI_NM_OPERACAO = "Paciente - Envio de Informações de Consulta";
                    hist.PAHI_DS_DESCRICAO = "Paciente: " + pac1.PACI_NM_NOME + " - Enviado em: " + item.AREA_DT_ENTRADA.Value.ToShortDateString();
                    Int32 voltaHist = baseApp.ValidateCreateHistorico(hist);
...
#pragma warning restore CS0219 // A variável é atribuída, mas seu valor nunca é usado
        ...
        public ActionResult SelecionarImprimirAtestado(Int32 id)
        {
            try
            {
                ...
                {
                    return RedirectToAction("GerarAtestadoPDFNovaAssina", "AreaPaciente");
                }
                else
                {
                    return RedirectToAction("GerarAtestadoPDFNova", "AreaPaciente");
                }
            }
            catch (Exception ex)
            {
        ...
        public ActionResult SelecionarImprimirSolicitacao(Int32 id)
        {
            try
            {
                ...
                if (conf.CONF_IN_ASSINA_DIGITAL_ATESTADO == 1)
                {
                    return RedirectToAction("GerarSolicitacaoPDFNovaAssina", "AreaPaciente");
                }
                else
                {
                    return RedirectToAction("GerarSolicitacaoPDFNova", "AreaPaciente");
                }
            }
            catch (Exception ex)
            {
        ...
        public ActionResult SelecionarImprimirPrescricao(Int32 id)
        {
            try
            {
                ...
                if (conf.CONF_IN_ASSINA_DIGITAL_ATESTADO == 1)
                {
                    ...
                    {
                        return RedirectToAction("GerarPrescricaoPDF_ComumNovaAssina", "AreaPaciente");
                    }
                    else
                    {
                        return RedirectToAction("GerarPrescricaoPDF_EspecialNovaAssina", "AreaPaciente");
                    }
                }
                else
                {
                    if (pres.TICO_CD_ID == 1)
                    {
                        return RedirectToAction("GerarPrescricaoPDF_ComumNova", "AreaPaciente");
                    }
                    else
                    {
                        return RedirectToAction("GerarPrescricaoPDF_EspecialNova", "AreaPaciente");
                    }
                }
            }
            catch (Exception ex)
        ...
        public ActionResult CarregarContratoDireto()
        {
            LOCACAO loca = locaApp.GetItemById((Int32)Session["IdLocacao"]);
            Session["VoltaContrato"] = 2;
            return RedirectToAction("CarregarContrato", "AreaPaciente");
        }
        ...
        public ActionResult IncluirComentario(NoticiaComentarioViewModel vm)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    Int32 volta = notApp.ValidateEdit(not, not, usu);
        ...
        public ActionResult VoltarVerLocacao()
        {
            if ((String)Session["Ativa"] == null)
            {
                return RedirectToAction("Logout", "ControleAcesso");
        ...
        public Int32  GerarAtestadoPDFNova()
        {
            try
            {
                Int32 idAss = (Int32)Session["IdAssinante"];
                ...
                String token = solic.PAAT_TK_TOKEN;

                // Atualiza atestado
                PACIENTE_ATESTADO atestado1 = baseApp.GetAtestadoById(solic.PAAT_CD_ID);
                ...
                headerTable = new PdfPTable(new float[] { 300f, 450f });
                ...
                cell.Colspan = 1;
                image = null;
                if (!String.IsNullOrEmpty(solic.PAAT_AQ_ARQUIVO_QRCODE))
                {
                    // 1. Pegamos o caminho gravado (ex: ~Imagens/1/Pacientes/...)
                    // 2. Removemos o "~" para obter o nome do Blob no Azure
                    String blobPath = solic.PAAT_AQ_ARQUIVO_QRCODE.Replace("~", "");
                ...
                cell = new PdfPCell(new Paragraph("Data/Hora da Emissão: " + solic.PAAT_DT_EMISSAO_COMPLETA.Value.ToLongDateString() + " " + solic.PAAT_DT_EMISSAO_COMPLETA.Value.ToLongTimeString() + " (GMT-3)", meuFont1Bold));
                cell.Border = 0;
                cell.Colspan = 4;
                cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                cell.HorizontalAlignment = Element.ALIGN_LEFT;
                table.AddCell(cell);
                cell = new PdfPCell(new Paragraph("Identificador: " + solic.PAAT_GU_GUID, meuFont1Bold));
                cell.Border = 0;
                cell.Colspan = 4;
                cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                cell.HorizontalAlignment = Element.ALIGN_LEFT;
                ...
                if (solic.TIPO_ATESTADO != null)
                {
                    cell = new PdfPCell(new Paragraph("Tipo de Atestado: ", meuFont1Bold))
                    {
                        VerticalAlignment = Element.ALIGN_MIDDLE,
                    ...
                    table.AddCell(cell);
                    cell = new PdfPCell(new Paragraph(solic.TIPO_ATESTADO.TIAT_NM_NOME, meuFont1))
                    {
                        VerticalAlignment = Element.ALIGN_MIDDLE,
                        HorizontalAlignment = Element.ALIGN_LEFT
                    };
                ...
                if (solic.PAAT_NM_TITULO != null)
                {
                    cell = new PdfPCell(new Paragraph("Finalidade: ", meuFont1Bold))
                    {
                        VerticalAlignment = Element.ALIGN_MIDDLE,
                        HorizontalAlignment = Element.ALIGN_LEFT
                    };
                    ...
                    cell.Border = PdfPCell.NO_BORDER;
                    table.AddCell(cell);
                    cell = new PdfPCell(new Paragraph(solic.PAAT_NM_TITULO, meuFont1))
                    {
                        VerticalAlignment = Element.ALIGN_MIDDLE,
                        HorizontalAlignment = Element.ALIGN_LEFT
                    };
                ...
                cell = new PdfPCell(new Paragraph("Destinatário: ", meuFont1Bold))
                {
                    VerticalAlignment = Element.ALIGN_MIDDLE,
                    HorizontalAlignment = Element.ALIGN_LEFT
                };
                ...
                cell.Border = PdfPCell.NO_BORDER;
                table.AddCell(cell);
                cell = new PdfPCell(new Paragraph(solic.PAAT_NM_DESTINO, meuFont1))
                {
                    VerticalAlignment = Element.ALIGN_MIDDLE,
                    HorizontalAlignment = Element.ALIGN_LEFT
                };
                ...
                Chunk chunk3 = new Chunk("A T E S T A D O", FontFactory.GetFont("Arial", 16, Font.NORMAL, BaseColor.BLACK));
                ...
                Chunk chunk1 = new Chunk(solic.PAAT_TX_TEXTO, FontFactory.GetFont("Arial", 9, Font.NORMAL, BaseColor.BLACK));
                pdfDoc.Add(chunk1);
        ...
        public Int32 GerarAtestadoPDFNovaAssina()
        {
            try
            {
                Int32 idAss = (Int32)Session["IdAssinante"];
                ...
                EMPRESA empresa = empApp.GetItemById(usuario.EMPR_CD_ID.Value);
                String token = solic.PAAT_TK_TOKEN;
                Int32? assina = solic.PAAT_IN_ASSINADO_DIGITAL;
                ...
                if (certificado == 1)
                {
                    String fraseAssina = "Documento assinado digitalmente em " + solic.PAAT_DT_EMISSAO_COMPLETA.Value.ToShortDateString() + " " + solic.PAAT_DT_EMISSAO_COMPLETA.Value.ToShortTimeString() + " conforme MP 2.200-2/01";
                ...
                if (solic.PAAT_IN_ASSINADO_DIGITAL == 0)
                {
                    // Ajusta espaço
                    line1 = new Paragraph("  ");
                    pdfDoc.Add(line1);
        ...
        public Int32 GerarSolicitacaoPDFNova()
        {
            try
            {
                ...
                if (conf.CONF_IN_LOGO_EMPRESA == 1)
                {
                    ...
                    cell1 = new PdfPCell(new Paragraph("Solicitações de Exames", meuFont2))
                    {
                        VerticalAlignment = Element.ALIGN_MIDDLE,
                        HorizontalAlignment = Element.ALIGN_CENTER
                    };
        ...
        public Int32 GerarPrescricaoPDF_ComumNova()
        {
            try
            {
                Int32 idAss = (Int32)Session["IdAssinante"];
                ...
                innerTableCell.Colspan = 4;
                footerTable.AddCell(innerTableCell);
                String msg = "(*) Para validar este documento use o código QR acima ou acesse " + conf.CONF_LK_LINK_VALIDACAO + " em Validar Documentos usando o token de acesso " + token;
                cell.AddElement(new Chunk(msg, FontFactory.GetFont("Arial", 8, Font.NORMAL, BaseColor.BLACK)));
                footerTable.AddCell(cell);
                ...
                cell = new PdfPCell(new Paragraph("Data/Hora da Emissão: " + solic.PAPR_DT_EMISSAO_COMPLETA.Value.ToLongDateString() + " " + solic.PAPR_DT_EMISSAO_COMPLETA.Value.ToLongTimeString() + " (GMT-3)", meuFont1Bold));
                cell.Border = 0;
                cell.Colspan = 4;
                cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                cell.HorizontalAlignment = Element.ALIGN_LEFT;
                table.AddCell(cell);
                cell = new PdfPCell(new Paragraph("Identificador: " + solic.PAPR_GU_GUID, meuFont1Bold));
                cell.Border = 0;
                cell.Colspan = 4;
                cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                cell.HorizontalAlignment = Element.ALIGN_LEFT;
                ...
                {
                    cell = new PdfPCell(new Paragraph(item.TIPO_FORMA.TIFO_NM_NOME, meuFont1))
                    {
                        VerticalAlignment = Element.ALIGN_MIDDLE,
                        HorizontalAlignment = Element.ALIGN_LEFT
                    };
                    ...
                    cell = new PdfPCell(new Paragraph(item.PAPI_NM_REMEDIO, meuFont1Bold))
                    {
                        VerticalAlignment = Element.ALIGN_MIDDLE,
                        HorizontalAlignment = Element.ALIGN_LEFT
                    };
                    ...
                    cell = new PdfPCell(new Paragraph(item.PAPI_NM_GENERICO, meuFont1))
                    {
                        VerticalAlignment = Element.ALIGN_MIDDLE,
                        HorizontalAlignment = Element.ALIGN_LEFT
                    };
                    ...
                    if (item.PAPI_NM_LABORATORIO != null)
                    {
                        cell = new PdfPCell(new Paragraph(item.PAPI_NM_LABORATORIO, meuFont1))
                        {
                            VerticalAlignment = Element.ALIGN_MIDDLE,
                            HorizontalAlignment = Element.ALIGN_LEFT
                        };
                    ...
                    if (item.PAPI_NM_APRESENTACAO != null)
                    {
                        cell = new PdfPCell(new Paragraph(item.PAPI_NM_APRESENTACAO, meuFont1))
                        {
                            VerticalAlignment = Element.ALIGN_MIDDLE,
                            HorizontalAlignment = Element.ALIGN_LEFT
                        };
                    ...
                    if (item.PAPI_NR_QUANTIDADE != null)
                    {
                        cell = new PdfPCell(new Paragraph(item.PAPI_NR_QUANTIDADE, meuFont1))
                        {
                            VerticalAlignment = Element.ALIGN_MIDDLE,
                            HorizontalAlignment = Element.ALIGN_LEFT
                        };
                    ...
                    cell = new PdfPCell(new Paragraph(item.PAPI_DS_POSOLOGIA, meuFont1))
                    {
                        VerticalAlignment = Element.ALIGN_MIDDLE,
                        HorizontalAlignment = Element.ALIGN_LEFT
                    };
                    ...
                    cell = new PdfPCell(new Paragraph(" ", meuFont1Bold))
                    {
                        VerticalAlignment = Element.ALIGN_MIDDLE,
                        HorizontalAlignment = Element.ALIGN_LEFT
                    };
        ...
        public Int32 GerarPrescricaoPDF_ComumNovaAssina()
        {
            try
            {
                Int32 idAss = (Int32)Session["IdAssinante"];
        ...
        public Int32 GerarPrescricaoPDF_EspecialNova()
        {
            try
            {
                Int32 idAss = (Int32)Session["IdAssinante"];
                ...
                // Rodape
                PdfPTable footerTable = new PdfPTable(new float[] { 300f, 300f });
                footerTable.WidthPercentage = 100;
                footerTable.HorizontalAlignment = 1;
                footerTable.SpacingBefore = 1f;
                footerTable.SpacingAfter = 1f;
                ...
                // Dados do comprador
                table1 = new PdfPTable(new float[] { 300f, 70f });
                table1.WidthPercentage = 100;
                table1.HorizontalAlignment = 0;
                table1.SpacingBefore = 1f;
                table1.SpacingAfter = 1f;

                cell = new PdfPCell(new Paragraph("IDENTIFICAÇÃO DO COMPRADOR", meuFont3Bold));
                cell.Border = 0;
                cell.Colspan = 2;
                cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                cell.HorizontalAlignment = Element.ALIGN_CENTER;
                ...
                cell = new PdfPCell(new Paragraph("Nome: ", meuFont1Bold));
                cell.Border = 0;
                cell.Colspan = 2;
                cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                cell.HorizontalAlignment = Element.ALIGN_LEFT;
                ...
                cell = new PdfPCell(new Paragraph("RG: ", meuFont1Bold));
                cell.Border = 0;
                cell.Colspan = 2;
                cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                cell.HorizontalAlignment = Element.ALIGN_LEFT;
                ...
                cell = new PdfPCell(new Paragraph("Endereço: ", meuFont1Bold));
                cell.Border = 0;
                cell.Colspan = 2;
                cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                cell.HorizontalAlignment = Element.ALIGN_LEFT;
                ...
                cell = new PdfPCell(new Paragraph("Telefone: ", meuFont1Bold));
                cell.Border = 0;
                cell.Colspan = 2;
                cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                cell.HorizontalAlignment = Element.ALIGN_LEFT;
                ...
                cell = new PdfPCell(new Paragraph("Cidade: ", meuFont1Bold));
                cell.Border = 0;
                cell.Colspan = 1;
                cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                cell.HorizontalAlignment = Element.ALIGN_LEFT;
                table1.AddCell(cell);
                cell = new PdfPCell(new Paragraph("UF: ", meuFont1Bold));
                cell.Border = 0;
                cell.Colspan = 1;
                cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                cell.HorizontalAlignment = Element.ALIGN_LEFT;
                ...
                // Fornecedor
                table1 = new PdfPTable(new float[] { 70f, 300f });
                table1.WidthPercentage = 100;
                table1.HorizontalAlignment = 0;
                table1.SpacingBefore = 1f;
                table1.SpacingAfter = 1f;

                cell = new PdfPCell(new Paragraph("IDENTIFICAÇÃO DO FORNECEDOR", meuFont3Bold));
                cell.Border = 0;
                cell.Colspan = 2;
                cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                cell.HorizontalAlignment = Element.ALIGN_CENTER;
                ...
                cell = new PdfPCell(new Paragraph("DATA", meuFont1Bold));
                cell.Border = 0;
                cell.Colspan = 2;
                ...
                cell = new PdfPCell(new Paragraph("ASSINATURA DO FARMACÊUTICO", meuFont1Bold));
                cell.Border = 0;
                cell.Colspan = 2;
                cell.Border = Rectangle.TOP_BORDER;
                cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                ...
                while (conta < 3)
                {
                    ...
                    // Monta endereco paciente
                    String endereco1 = String.Empty;
                    String enderecoCont1 = String.Empty;
                    if (paciente.PACI_NM_ENDERECO != null)
                    {
                        endereco1 += paciente.PACI_NM_ENDERECO;
                        if (paciente.PACI_NR_NUMERO != null)
                        {
                            endereco1 += " " + paciente.PACI_NR_NUMERO;
                        }
                        if (paciente.PACI_NR_COMPLEMENTO != null)
                        {
                            endereco1 += " " + paciente.PACI_NR_COMPLEMENTO;
                        }
                        if (paciente.PACI_NM_BAIRRO != null)
                        {
                            enderecoCont1 += paciente.PACI_NM_BAIRRO;
                        }
                        if (paciente.PACI_NM_CIDADE != null)
                        {
                            enderecoCont1 += " - " + paciente.PACI_NM_CIDADE;
                        }
                        if (paciente.UF != null)
                        {
                            enderecoCont1 += " - " + paciente.UF.UF_SG_SIGLA;
                        }
                        if (paciente.PACI_NR_CEP != null)
                        {
                            enderecoCont1 += " - " + paciente.PACI_NR_CEP;
                        }
                    }


                    // Cabeçalho Base
                    PdfPTable table = new PdfPTable(new float[] { 300f, 300f });
                    table.WidthPercentage = 100;
                    ...
                    cell.HorizontalAlignment = Element.ALIGN_LEFT;
                    table1.AddCell(cell);
                    cell = new PdfPCell(new Paragraph(usuario.USUA_NM_NOME, meuFont1));
                    cell.Border = 0;
                    cell.Colspan = 1;
                    cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                    cell.HorizontalAlignment = Element.ALIGN_LEFT;
                    ...
                    cell = new PdfPCell(new Paragraph("CRM: ", meuFont1Bold));
                    cell.Border = 0;
                    cell.Colspan = 1;
                    cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                    cell.HorizontalAlignment = Element.ALIGN_LEFT;
                    table1.AddCell(cell);
                    cell = new PdfPCell(new Paragraph(usuario.USUA_NR_CLASSE, meuFont1));
                    cell.Border = 0;
                    cell.Colspan = 1;
                    cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                    cell.HorizontalAlignment = Element.ALIGN_LEFT;
                    ...
                    cell.HorizontalAlignment = Element.ALIGN_LEFT;
                    table1.AddCell(cell);
                    cell = new PdfPCell(new Paragraph(endereco, meuFont1));
                    cell.Border = 0;
                    cell.Colspan = 1;
                    cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                    cell.HorizontalAlignment = Element.ALIGN_LEFT;
                    ...
                    cell.HorizontalAlignment = Element.ALIGN_LEFT;
                    table1.AddCell(cell);
                    cell = new PdfPCell(new Paragraph(enderecoCont, meuFont1));
                    cell.Border = 0;
                    cell.Colspan = 1;
                    cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                    cell.HorizontalAlignment = Element.ALIGN_LEFT;
                    ...
                    cell.HorizontalAlignment = Element.ALIGN_LEFT;
                    table1.AddCell(cell);
                    cell = new PdfPCell(new Paragraph(usuario.USUA_NR_CELULAR, meuFont1));
                    cell.Border = 0;
                    cell.Colspan = 1;
                    cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                    cell.HorizontalAlignment = Element.ALIGN_LEFT;
                    ...
                    cell = new PdfPCell(new Paragraph("RECEITUÁRIO DE CONTROLE ESPECIAL", meuFont3Bold));
                    cell.Border = 0;
                    cell.Colspan = 2;
                    cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                    cell.HorizontalAlignment = Element.ALIGN_CENTER;
                    ...
                    cell.HorizontalAlignment = Element.ALIGN_LEFT;
                    table1.AddCell(cell);
                    cell = new PdfPCell(new Paragraph(solic.PAPR_DT_EMISSAO.Value.ToShortDateString(), meuFont1));
                    cell.Border = 0;
                    cell.Colspan = 1;
                    cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                    cell.HorizontalAlignment = Element.ALIGN_LEFT;
                    ...
                    cell = new PdfPCell(new Paragraph("1a via - Farmácia", meuFont1Bold));
                    cell.Border = 0;
                    cell.Colspan = 2;
                    cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                    cell.HorizontalAlignment = Element.ALIGN_LEFT;
                    ...
                    cell = new PdfPCell(new Paragraph("2a via - Paciente", meuFont1Bold));
                    cell.Border = 0;
                    cell.Colspan = 2;
                    cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                    cell.HorizontalAlignment = Element.ALIGN_LEFT;
                    ...
                    cell = new PdfPCell(new Paragraph("ASSINATURA", meuFont1Bold));
                    cell.Border = 0;
                    cell.Colspan = 2;
                    cell.Border = Rectangle.TOP_BORDER;
                    cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                    ...
                    cell = new PdfPCell(new Paragraph("Nome do Paciente: ", meuFont1Bold));
                    cell.Border = 0;
                    cell.Colspan = 1;
                    cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                    cell.HorizontalAlignment = Element.ALIGN_LEFT;
                    table.AddCell(cell);
                    cell = new PdfPCell(new Paragraph(paciente.PACI_NM_NOME, meuFont1Bold));
                    cell.Border = 0;
                    cell.Colspan = 1;
                    cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                    cell.HorizontalAlignment = Element.ALIGN_LEFT;
                    ...
                    cell = new PdfPCell(new Paragraph("CPF: ", meuFont1));
                    cell.Border = 0;
                    cell.Colspan = 1;
                    cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                    cell.HorizontalAlignment = Element.ALIGN_LEFT;
                    table.AddCell(cell);
                    cell = new PdfPCell(new Paragraph(paciente.PACI_NR_CPF, meuFont1));
                    cell.Border = 0;
                    cell.Colspan = 1;
                    cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                    cell.HorizontalAlignment = Element.ALIGN_LEFT;
                    ...
                    cell = new PdfPCell(new Paragraph("Endereço: ", meuFont1));
                    cell.Border = 0;
                    cell.Colspan = 1;
                    cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                    cell.HorizontalAlignment = Element.ALIGN_LEFT;
                    table.AddCell(cell);
                    cell = new PdfPCell(new Paragraph(endereco1, meuFont1));
                    cell.Border = 0;
                    cell.Colspan = 1;
                    cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                    cell.HorizontalAlignment = Element.ALIGN_LEFT;
                    ...
                    cell.HorizontalAlignment = Element.ALIGN_LEFT;
                    table.AddCell(cell);
                    cell = new PdfPCell(new Paragraph(enderecoCont1, meuFont1));
                    cell.Border = 0;
                    cell.Colspan = 1;
                    cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                    cell.HorizontalAlignment = Element.ALIGN_LEFT;
        ...
        public Int32 GerarPrescricaoPDF_EspecialNovaAssina()
        {
            try
            {
                Int32 idAss = (Int32)Session["IdAssinante"];
...
#pragma warning restore CS0219 // A variável é atribuída, mas seu valor nunca é usado
        ...
        public ActionResult CarregarContrato()
        {
            try
            {
                ...
                // Recupera informações
                LOCACAO item = locaApp.GetItemById((Int32)Session["IdLocacao"]);
        ...
        [HttpGet]
        public ActionResult MontarTelaAreaPacienteVer()
        {
            try
            {
                // Verifica se tem usuario logado
                ...
                ViewBag.AreaDoc = areaDoc;

                // Mensagens
                if (Session["MensArea"] != null)
                {
                    if ((Int32)Session["MensArea"] == 1)
                    {
        ...
        public ActionResult FiltrarAreaPaciente(AreaPacienteViewModel vm)
        {
            try
            {
                ...
                // Sucesso
                Session["MensArea"] = null;
                listaMasterArea = volta.Item2;
                Session["ListaAreaPaciente"] = volta.Item2;
                return RedirectToAction("MontarTelaAreaPacienteVer");
            }
            catch (Exception ex)
            {
        ...
        public ActionResult RetirarFiltroAreaPaciente()
        {
            try
            {
                if ((String)Session["Ativa"] == null)
        ...
        [HttpGet]
        public ActionResult AreaPacienteItemVer(Int32 id)
        {
            try
            {
                // Verifica se tem usuario logado
                ...
                AREA_PACIENTE item = areaApp.GetItemById(id);
                PACIENTE pac = baseApp.GetItemById(item.PACI_CD_ID.Value);

                AreaPacienteViewModel vm = Mapper.Map<AREA_PACIENTE, AreaPacienteViewModel>(item);
                Session["IdAreaPaciente"] = item.AREA_CD_ID;
                Session["AreaPaciente"] = item;
                Session["IdPaciente"] = item.PACI_CD_ID;

                // Grava Acesso
                ControleAcessoMetodo grava = new ControleAcessoMetodo(aceApp);
                Int32 voltaX = grava.GravaAcesso(usuario.USUA_CD_ID, usuario.ASSI_CD_ID, "AREA_PACIENTE_VER", "AreaPaciente", "AreaPacienteItemVer");
                return View(vm);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult VoltarAreaPacienteVer()
        {
            if ((String)Session["Ativa"] == null)
            {
                return RedirectToAction("Logout", "ControleAcesso");
            ...
            AREA_PACIENTE velha = (AREA_PACIENTE)Session["AreaPaciente"];
            AREA_PACIENTE area = areaApp.GetItemById(velha.AREA_CD_ID);
        ...
        public ActionResult ProcessarMarcacaoConsultaPlano()
        {
            if ((String)Session["Ativa"] == null)
            {
                return RedirectToAction("Logout", "ControleAcesso");
        ...
        [HttpGet]
        public ActionResult ProcessarMarcacaoConsulta(Int32 id)
        {
            try
            {
                // Verifica se tem usuario logado
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ProcessarMarcacaoConsulta(AREA_PACIENTE area)
        {
            if ((String)Session["Ativa"] == null)
            {
                return RedirectToAction("Logout", "ControleAcesso");
        ...
        public ActionResult FecharMarcacaoConsulta()
        {
            if ((String)Session["Ativa"] == null)
            {
                return RedirectToAction("Logout", "ControleAcesso");
        ...
        public ActionResult ProcessarInfoConsultaPlano(Int32 id)
        {
            ...
            try
            {
                ...
                if (area.AREA_TX_CONTEUDO != null)
                {
                    String velho = anam.PAAM_TX_TEXTO_LIVRE;
                    String novo = "=== Informação Prévia de Consulta === " + Environment.NewLine + area.AREA_TX_CONTEUDO;
                ...
                // Trata anexos
                if (area.AREA_PACIENTE_ANEXO.Count > 0)
                {
                    foreach (AREA_PACIENTE_ANEXO item in area.AREA_PACIENTE_ANEXO)
                    {
                        Int32 volta1 = UploadFilePaciente(pac, item);
                    }
                }
        ...
        public Int32 UploadFilePaciente(PACIENTE pac, AREA_PACIENTE_ANEXO item)
        {
            try
            {
                ...
                // 1. Recupera as configurações do Azure (Ajuste conforme o objeto de configuração que você tiver no escopo)
                ...
                String containerNome = "rti-datacontainer"; // O nome do seu container no Azure Blob Storage

                // 2. Cria o cliente do serviço de Blob
                ...
                // 3. Monta as estruturas de caminhos virtuais (sem o Server.MapPath, pois na nuvem usamos caminhos relativos/blobs)
                String extensao = Path.GetExtension(item.APAN_NM_TITULO);

                // Removemos a barra inicial para o Azure não criar uma pasta fantasma vazia no início do caminho
                ...
                // 5. Executa a cópia diretamente dentro do Storage da Azure (Sem trafegar o arquivo pelo seu servidor web)
                if (blobOrigem.Exists())
                {
                    // O método StartCopyFromUri inicia uma cópia assíncrona server-side na Azure
                    blobDestino.StartCopyFromUri(blobOrigem.Uri);
                }
                else
                {
                    throw new FileNotFoundException($"O arquivo de origem não foi encontrado no Azure Storage: {blobOrigemPath}");
                }
        ...
        [HttpGet]
        public ActionResult ProcessarInfoConsulta(Int32 id)
        {
            try
            {
                // Verifica se tem usuario logado
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ProcessarInfoConsulta(AREA_PACIENTE area)
        {
            if ((String)Session["Ativa"] == null)
            {
                return RedirectToAction("Logout", "ControleAcesso");
        ...
        [HttpGet]
        public ActionResult EnviarEMailPaciente()
        {
            try
            {
                // Verifica se tem usuario logado
                ...
                Int32 idAss = (Int32)Session["IdAssinante"];
                Session["ModuloAtual"] = "Paciente - Envio de E-Mail";
                ...
                {
                    Session["MensArea"] = 400;
                    return RedirectToAction("MontarTelaAreaPacienteVer");
                }

                // Recupera contato
                List<PACIENTE> lp = CarregaPaciente();
                ViewBag.Pacientes = new SelectList(lp, "PACI__CD_ID", "PACI_NM_NOME");
        ...
        public async Task<ActionResult> EnviarEMailPaciente(MensagemViewModel vm)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    // Monta Mensagem
                    vm.NOME = pac.PACI_NM_NOME;
                    vm.ID = pac.PACI__CD_ID;
                    vm.MODELO = pac.PACI_NM_EMAIL;
                    vm.MENS_DT_CRIACAO = DateTime.Today.Date;
                    vm.MENS_IN_TIPO = 1;
                    vm.MENS_IN_TIPO_EMAIL = 1;
                    vm.TIPO_ENVIO = 1;
                    vm.MENS_NM_CAMPANHA = pac.PACI_NM_EMAIL;
                    vm.MENS_NM_NOME = "Mensagem para Paciente: " + pac.PACI_NM_NOME;

                    // Prepara cabeçalho
                    String cab = "Prezado Sr(a). <b>" + pac.PACI_NM_NOME + "</b><br />";
                    ...
                    NetworkCredential net = new NetworkCredential(conf.CONF_NM_SENDGRID_LOGIN, conf.CONF_NM_SENDGRID_PWD);
                    EmailAzure mensagem = new EmailAzure();
                    mensagem.ASSUNTO = "Mensagem para Paciente - " + pac.PACI_NM_NOME;
                    mensagem.CORPO = emailBody;
                    mensagem.DEFAULT_CREDENTIALS = false;
                    mensagem.EMAIL_TO_DESTINO = pac.PACI_NM_EMAIL;
                    mensagem.NOME_EMISSOR_AZURE = emissor;
                    ...
                    hist.PAHI_DT_DATA = DateTime.Now;
                    hist.PAHI_IN_TIPO = 2;
                    hist.PAHI_IN_CHAVE = null;
                    hist.PAHI_NM_OPERACAO = "Paciente - Envio de Mensagem E-Mail";
                    hist.PAHI_DS_DESCRICAO = "Mensagem para Paciente " + pac.PACI_NM_NOME;
                    Int32 voltaHist = baseApp.ValidateCreateHistorico(hist);

                    // Mensagem do CRUD
                    Session["MsgCRUD"] = "Foi enviada mensagem de e-mail para o paciente " + pac.PACI_NM_NOME.ToUpper();
                    Session["MensArea"] = 61;
        ...
        public async Task<ActionResult> ProcessarResultadoExame(Int32 id)
        {
            if ((String)Session["Ativa"] == null)
            {
                return RedirectToAction("Logout", "ControleAcesso");
            ...
            try
            {
                ...
                // Grava exame
                Int32 volta1 = baseApp.ValidateCreateExame(exame);
                ...
                {
                    fisicoG.PAEF_TX_RESULTADOS += frase + "\r\n" + "*** " + area.AREA_DT_DATA_EXAME.Value.ToLongDateString() + " ***" + "\r\n";
                }
                else
                {
                    fisicoG.PAEF_TX_RESULTADOS += "\r\n\r\n" + frase + "\r\n" + "*** " + area.AREA_DT_DATA_EXAME.Value.ToLongDateString() + " ***" + "\r\n";
                }
                fisicoG.PAEF_TX_RESULTADOS += ">>" + area.AREA_NM_EXAME + "<<" + "\r\n";
                fisicoG.PAEF_TX_RESULTADOS += area.AREA_TX_CONTEUDO;
                ...
                foreach (AREA_PACIENTE_ANEXO item in docs)
                {
                    ...
                    if (item.APAN_NM_TITULO.Length > 250)
                    {
                        Session["MensArea"] = 91;
                        return RedirectToAction("MontarTelaAreaPacienteVer");
                    }
                    ...
                    foto.PAEO_AQ_ARQUIVO = "~" + blobDestinoPath + item.APAN_NM_TITULO;
                    ...
                    Int32 volta2 = await baseApp.ValidateEditExameAsync(exame);
                }
        ...
        public DTO_Paciente_Exame MontarExameDTOObj(PACIENTE_EXAMES l)
        {
            using (var context = new CRMSysDBEntities())
            {
                var mediDTO = new DTO_Paciente_Exame()
                {
                    ASSI_CD_ID = l.ASSI_CD_ID,
                    LABS_CD_ID = l.LABS_CD_ID,
                    PACO_CD_ID = l.PACO_CD_ID,
                    PAEX_AQ_ARQUIVO = l.PAEX_AQ_ARQUIVO,
                    PAEX_CD_ID = l.PAEX_CD_ID,
                    PAEX_DS_COMENTARIOS = l.PAEX_DS_COMENTARIOS,
                    PAEX_DS_DIAGNOSTICO = l.PAEX_DS_DIAGNOSTICO,
                    PAEX_DT_DATA = l.PAEX_DT_DATA,
                    PAEX_DT_DUMMY = l.PAEX_DT_DUMMY,
                    PAEX_IN_ATIVO = l.PAEX_IN_ATIVO,
                    PAEX_IN_TIPO = l.PAEX_IN_TIPO,
                    PAEX_NM_NOME = l.PAEX_NM_NOME,
        ...
        public async Task<ActionResult> ProcessarResultadoExameTela(Int32 id)
        {
            try
            {
                ...
                // Grava Acesso
                ControleAcessoMetodo grava = new ControleAcessoMetodo(aceApp);
                Int32 voltaX = grava.GravaAcesso(usuario.USUA_CD_ID, usuario.ASSI_CD_ID, "PACIENTE_EXAME_EDITAR", "Paciente", "EditarExame");
                return View(vm);
            }
            catch (Exception ex)
            {
        ...
        public FileResult DownloadPacienteExameOld(Int32 id)
        {
            try
            {
                PACIENTE_EXAME_ANEXO item = baseApp.GetExameAnexoById(id);
        ...
        public async Task<JsonResult> CancelarConsultaJson(Int32 id, String justificativa)
        {
            try
            {
                ...
                {
                    LOG_DT_DATA = DateTime.Now,
                    ASSI_CD_ID = pac.ASSI_CD_ID,
                    USUA_CD_ID = pac.USUA_CD_ID.Value,
                    LOG_NM_OPERACAO = "Paciente - Consulta - Cancelamento",
                    LOG_IN_ATIVO = 1,
                ...
                // Grava historico
                PACIENTE_HISTORICO hist = new PACIENTE_HISTORICO();
                hist.ASSI_CD_ID = pac.ASSI_CD_ID;
                hist.USUA_CD_ID = pac.USUA_CD_ID.Value;
                hist.PACI_CD_ID = item.PACI_CD_ID;
                hist.PAHI_DT_DATA = DateTime.Now;
                hist.PAHI_IN_TIPO = 10;
                hist.PAHI_IN_CHAVE = item.PACO_CD_ID;
                ...
                if (pac.USUARIO.USUA_NM_EMAIL != null)
                {
                    Int32 voltaCons = await EnviarEMailConsulta(item, 7);
                }

                urlRedirecionamento = Url.Action("MontarTelaAreaPaciente", "AreaPaciente");
        ...
        public async Task<JsonResult> ConfirmarConsultaJson(Int32 id)
        {
            try
            {
                ...
                PACIENTE_HISTORICO hist = new PACIENTE_HISTORICO();
                hist.ASSI_CD_ID = pac.ASSI_CD_ID;
                hist.USUA_CD_ID = pac.USUA_CD_ID;
                hist.PACI_CD_ID = item.PACI_CD_ID;
                hist.PAHI_DT_DATA = DateTime.Now;
                hist.PAHI_IN_TIPO = 10;
                hist.PAHI_IN_CHAVE = item.PACO_CD_ID;
                ...
                if (pac.USUARIO.USUA_NM_EMAIL != null & conf.CONF_IN_ENVIA_CONFIRMACAO == 1)
                {
                    Int32 voltaCons = await EnviarEMailConsulta(item, 6);
                }
